...
namespace ctl {
constexpr size_t posix_domain_code = 0x1;
enum class posix_condition : size_t {
...
constexpr size_t win32_domain_code = 0x2;
enum class win32_condition : size_t {
...
namespace detail {
inline bool is_equivalent(posix_condition a, win32_condition b) {
  return false;  // TODO
}

inline bool is_equivalent(size_t domain_a,
                          size_t condition_a,
                          size_t domain_b,
                          size_t condition_b) {
  posix_condition posix;
  win32_condition win32;

  switch (domain_a) {
    case posix_domain_code: posix = static_cast<posix_condition>(condition_a); break;
    case win32_domain_code: win32 = static_cast<win32_condition>(condition_a); break;
    default: return false;
  }

  switch (domain_b) {
    case posix_domain_code: posix = static_cast<posix_condition>(condition_b); break;
    case win32_domain_code: win32 = static_cast<win32_condition>(condition_b); break;
    default: return false;
  }

  return detail::is_equivalent(posix, win32);
}
...
class code {
 public:
  ...
  void set_domain_bits(uint64_t v) noexcept {
    bits_ &= ~domain_mask;
    bits_ |= v << domain_shift;
  }
  void set_domain_bits(code c) noexcept {
    bits_ &= ~domain_mask;
  ...
  void set_incident_bits(uint64_t v) noexcept {
    bits_ &= ~incident_mask;
    bits_ |= v << incident_shift;
  }
  void set_incident_bits(code c) noexcept {
    bits_ &= ~incident_mask;
    bits_ |= c.bits_ & incident_mask;
  }
  ...
  bool same_incident(code that) const noexcept {
    return (bits_ & incident_compare_mask) == (that.bits_ & incident_compare_mask);
  }
...
class domain {
 public:
  explicit domain(size_t code) : code_{code} {}
  ...
  virtual std::string_view message(error) const noexcept = 0;
  virtual std::source_location location(error) const noexcept = 0;
  virtual bool equivalent(error, condition) const noexcept = 0;

 protected:
  friend class error;
  error make_error(detail::code code) const noexcept;
  size_t incident_code(error) const noexcept;
  size_t condition_code(error) const noexcept;
  size_t domain_code(error) const noexcept;

  friend class condition;
  condition make_condition(detail::code code) const noexcept;
  size_t condition_code(condition) const noexcept;
  size_t domain_code(condition) const noexcept;

  size_t domain_code() const noexcept { return code_; }
...
class error final {
 ...
 private:
  friend class domain;
  explicit error(const domain *d, detail::code c) : domain_{d}, code_{c} {}

  bool equivalent(condition condition) const noexcept {
    return code_.same_condition(condition.code_);
  }

  friend bool operator==(error a, condition b) noexcept {
    return a.equivalent(b) || a.domain_->equivalent(a, b);
  }

  friend bool operator==(condition a, error b) noexcept {
    return b.equivalent(a) || b.domain_->equivalent(b, a);
  }
...
inline error domain::make_error(detail::code code) const noexcept {
  return error{this, code};
}

inline size_t domain::incident_code(error error) const noexcept {
  return error.code_.incident_bits();
}

inline size_t domain::condition_code(error error) const noexcept {
  return error.code_.condition_bits();
}

inline size_t domain::domain_code(error error) const noexcept {
  return error.code_.domain_bits();
}

inline condition domain::make_condition(detail::code code) const noexcept {
  return condition{code};
}

inline size_t domain::condition_code(condition condition) const noexcept {
  return condition.code_.condition_bits();
}

inline size_t domain::domain_code(condition condition) const noexcept {
...
class posix_domain final : public domain {
 public:
  posix_domain() : domain{posix_domain_code}, name_{"posix"} {}
  std::string_view name() const noexcept override { return name_; }

  std::string_view message(error error) const noexcept override {
    return incidents_[incident_code(error)].message;
  }

  std::source_location location(error error) const noexcept override {
    return incidents_[incident_code(error)].location;
  }

  bool equivalent(error error, condition condition) const noexcept override {
    return false;  // TODO: use detail::is_equivalent
  }

  error raise(posix_condition condition, std::source_location location, std::string message = {}) {
    auto current = next_incident_++;
    next_incident_ %= incidents_.size();
    incidents_[current] = detail::incident_entry{std::move(location), std::move(message)};
    return make_error(detail::code{static_cast<uint64_t>(domain_code()),
                                   static_cast<uint64_t>(condition),
                                   static_cast<uint64_t>(current)});
  }

  condition expect(posix_condition condition) {
    return make_condition(detail::code{static_cast<uint64_t>(domain_code()),
                                       static_cast<uint64_t>(condition),
                                       static_cast<uint64_t>(0)});
  }

 private:
  static const std::array<detail::condition_entry, 125> categories_;
  std::array<detail::incident_entry, 128> incidents_;
  std::size_t next_incident_ = 0;
  std::string name_;
};

// In win32 case there is an indirect mapping between the value of the condition
// and the value of condition, as represented by `condition_table_`.
//
class win32_domain final : public domain {
 public:
  ;

  explicit win32_domain() : domain{win32_domain_code}, name_{"win32"} {}
  std::string_view name() const noexcept override { return name_; }
  ...
  error raise(win32_condition condition, std::source_location location, std::string message = {}) {
    auto current = next_incident_++;
    next_incident_ %= incidents_.size();
    incidents_[current] = detail::incident_entry{std::move(location), std::move(message)};
    return make_error(detail::code{static_cast<uint64_t>(domain_code()),
                                   condition_table_.at(static_cast<uint64_t>(condition)),
                                   static_cast<uint64_t>(current)});
  }

  condition expect(win32_condition condition) {
    return make_condition(detail::code{static_cast<uint64_t>(domain_code()),
                                       condition_table_.at(static_cast<uint64_t>(condition)),
                                       static_cast<uint64_t>(0)});
  }
 ...
 private:
  static const std::array<detail::condition_entry, 200> categories_;
