...
enum class posix_condition : int {
...
enum class win32_condition : int {
...
// Bit container implementation for 64-bit machines.
//
class code {
 public:
...
class domain {
 public:
    ...
    virtual std::string_view message(hazard) const noexcept = 0;
    virtual std::source_location location(hazard) const noexcept = 0;
    virtual bool equivalent(hazard, condition) const noexcept = 0;

 protected:
    friend class hazard;
    hazard make_hazard(detail::code code) const noexcept;
    size_t incident_code(hazard) const noexcept;
    size_t category_code(hazard) const noexcept;
    size_t domain_code(hazard) const noexcept;

    friend class condition;
    condition make_condition(detail::code code) const noexcept;
    size_t category_code(condition) const noexcept;
...
class hazard final {
 ...
 private:
    ...
    bool equivalent(condition condition) const noexcept {
        return code_.same_category(condition.code_);
    }

    friend bool operator==(hazard a, condition b) noexcept {
        return a.equivalent(b) || a.domain_->equivalent(a, b);
    }

    friend bool operator==(condition a, hazard b) noexcept {
...
inline hazard domain::make_hazard(detail::code code) const noexcept {
    return hazard{this, code};
}

inline size_t domain::incident_code(hazard hazard) const noexcept {
    return hazard.code_.incident_bits();
}

inline size_t domain::category_code(hazard hazard) const noexcept {
    return hazard.code_.category_bits();
}

inline size_t domain::domain_code(hazard hazard) const noexcept {
    return hazard.code_.domain_bits();
}
...
inline size_t domain::category_code(condition condition) const noexcept {
    return condition.code_.category_bits();
}
...
class posix_domain final : public domain {
 public:
    ...
    std::string_view message(hazard hazard) const noexcept override {
        return incidents_[incident_code(hazard)].message;
    }

    std::source_location location(hazard hazard) const noexcept override {
        return incidents_[incident_code(hazard)].location;
    }

    bool equivalent(hazard hazard, condition condition) const noexcept override {
        return false;  // TODO: use detail::is_equivalent
    }

    hazard raise(posix_condition condition,
                 std::source_location location,
                 std::string message = {}) {
        ...
        return make_hazard(detail::code{static_cast<uint64_t>(domain_code()),
...
// and the value of category, as represented by `category_table_`.
//
class win32_domain final : public domain {
 public:
    ...
    hazard raise(win32_condition condition,
                 std::source_location location,
                 std::string message = {}) {
        auto current = next_incident_++;
        next_incident_ %= incidents_.size();
